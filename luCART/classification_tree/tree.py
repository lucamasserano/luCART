...
class Node:
    ...
    def decision_constructor(self, data_source_type: str):
        # I use node keys to construct decision paths for SQL DB and node values for DataFrames
        # this is done for formatting purposes when evaluating the decision path
        # TODO: rewrite Node class to have split_variable, relation, split_point as key, relation, value and use this
        #  method to return the correctly formatted decision path constructor --> more general and less hard-coded
        #  Note that this modification has dependencies to fix basically everywhere
        ...
        if data_source_type == "dataframe":
            ...
            raise ValueError(f"Only DataFrames and PostgreSQL Databases are supported, got {data_source_type}")
...
class ClassificationTree:
    ...
    def __init__(self,
        ...
        if data_source_type == "SQL":
            ...
            self.data_handler = data_handler.DataframeHandler(impurity_function=impurity_function)
        else:
    ...
    def label_encoder(self):
        return self.data_handler.label_encoder

    @label_encoder.setter
    def label_encoder(self, label_encoder):
        self.data_handler.label_encoder = label_encoder
    ...
    def remove_node(self, node) -> None:
        # is this needed?
        raise NotImplementedError("Is this method needed?")
    ...
    def decision_path(self, target_node: Node, current_node=None, tree_keys=None):
        ...
        if current_node.key == target_node.key:
            if current_node.key == "root":
                ...
                return [target_node.decision_constructor(self.data_source_type)]

        # target_node key (split_covariate <>== split_point) is in left branch
        # TODO: what if key is both in left and right branch? Is this possible? Probably yes with categorical features
        if current_node.children_keys[target_node.key] == "left":
            if current_node.key == "root":
                ...
                return [current_node.decision_constructor(self.data_source_type)] + \
                       self.decision_path(target_node=target_node,
                                          current_node=current_node.left,
                                          tree_keys=tree_keys)
        else:  # right branch
            if current_node.key == "root":
    ...
    def covariate_impurity_reduction(self,
        ...
        impurity_reduction_constructors = \
            self.data_handler.impurity_reduction_constructors(covariate_name=covariate_name,
                                                              label=self.label,
        ...
        logging.debug(f"{covariate_name} best split point: {impurity_reduction_constructors[5][max_reduction_index]}.\
                      Impurity reduction: {impurity_reductions[max_reduction_index]}")

        return impurity_reduction_constructors[5][max_reduction_index], impurity_reductions[max_reduction_index]
    ...
    def pruning_step(self, node: Node, decision_path=None):
        ...
        if node.key == "root":
            ...
            decision_path += [node.decision_constructor(self.data_source_type)]
    ...
    def fit(self, data: Union[str, pd.Dataframe], label: str) -> None:
        ...
        self.label_encoder = self.data_handler.encode_label(label=label, data=data)
